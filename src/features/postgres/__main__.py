...
DEFAULT_SHARED_ASSETS_INPUT = REPOSITORY_ROOT / "data/manifests/r2_shared_assets.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--shared-assets-input",
        type=Path,
        default=DEFAULT_SHARED_ASSETS_INPUT,
        help=f"Verified shared R2 asset manifest (default: {DEFAULT_SHARED_ASSETS_INPUT})",
    )
...
def main() -> None:
    ...
    motion_count, asset_count, shared_asset_count = import_catalog(
        database_url,
        args.input,
        args.assets_input,
    ...
    print(
        f"Imported {motion_count} {'record' if motion_count == 1 else 'records'} "
        f"into public.motions, "
        f"{asset_count} {'record' if asset_count == 1 else 'records'} "
        f"into public.motion_assets, and "
        f"{shared_asset_count} {'record' if shared_asset_count == 1 else 'records'} "
        f"into public.shared_assets"
    )
